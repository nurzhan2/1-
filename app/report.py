...
def build_report(
    ...
    for point in points:
        ...
        if value is not None:
            if value > 100:
                notes.append(
                    f"КПД не выводится: расчёт даёт {value:.0f}%, что физически "
                    "невозможно. Узлы учёта тепла и газа несопоставимы — часть "
                    "топлива не заведена в ЛЭРС либо у узлов разный расчётный час"
                )
            elif value < 60:
                notes.append(

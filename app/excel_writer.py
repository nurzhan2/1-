...
def _write_table(ws, report: ReportData) -> int:
    ...
    for row in report.rows:
        ...
        eff = ws.cell(
            row_idx,
            5,
            # КПД выше 100 % физически невозможен: это всегда признак того, что
            # узлы учёта тепла и газа несопоставимы (часть топлива не заведена
            # в ЛЭРС либо у узлов разный расчётный час). Такое значение не
            # выводим — оно не характеризует работу котельной. Гкал и Нм3
            # остаются в своих колонках, причина указана в примечании.
            f'=IFERROR(IF(B{row_idx}/(C{row_idx}*{NAME_CALORIFIC}/1000000)*100>100,"",'
            f'B{row_idx}/(C{row_idx}*{NAME_CALORIFIC}/1000000)*100),"")',
        )
